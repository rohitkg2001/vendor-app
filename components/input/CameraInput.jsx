...
export default function CameraInput({ isCameraOpen, setIsCameraOpen, handleImageCapture }) {
    ...
    const handleCapture = async () => {
        if (cameraRef.current && location) {
            ...
            setPhotos(prev => [photoData, ...prev].slice(0, 5)); // Keep only last 5 photos
        }
    };
        ...
        <Modal visible={isCameraOpen} animationType="slide" onRequestClose={() => setIsCameraOpen(false)}>
            <View style={styles.cameraContainer}>
                ...
                <View style={styles.watermark}>
                    <Text style={styles.watermarkText}>Powered by Dashandots Technology</Text>
                    <Text style={styles.watermarkText}>📍 {location?.latitude}, {location?.longitude}</Text>
                ...
                <View style={styles.controls}>
                    ...
                    {
                        photos.length >= 2 ? (
                            <TouchableOpacity onPress={() => handleImageCapture(photos)} style={styles.retakeButton}>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <View style={styles.photoItem}>
                            ...
                            <Text style={styles.photoText}>{item.lat.toFixed(4)}, {item.long.toFixed(4)}</Text>
